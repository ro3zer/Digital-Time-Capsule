...
#         if not file.get('unlocked', False) and current_date < unlock_date:
#             return jsonify({
#                 'error': '이 파일은 아직 잠겨있습니다',
#                 'message': f'파일은 {unlock_date.strftime("%Y년 %m월 %d일 %H시 %M분")}에 열람 가능합니다.',
#                 'unlock_date': file['unlock_date']
#             }), 403
...
#         if user_id not in allowed_users:
#             raise TimeCapsuleError("접근 권한이 없습니다", 403)
...
#         response = requests.get(download_url, headers=headers, stream=True)
#         if response.status_code != 200:
#             raise TimeCapsuleError("다운로드에 실패했습니다", response.status_code)
